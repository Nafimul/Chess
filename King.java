		...
		if (!hasMoved) {
			...
			if (rookStartingSpace.hasPiece("rook")) {
				if (!rookStartingSpace.getPiece().hasMoved) {
					legalMoves.add(rookStartingLoc); //if the player decides to move to the location of their own rook, 
					//the movePiece function in the chess class will automaticaly cause the player to castle
				}
			}
		}
	}
